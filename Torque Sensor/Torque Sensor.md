...
该设备测量的是绕传感器中心轴的扭矩，不是任意方向的按压力。测试时固定传感器一侧，对另一侧绕中心轴施加并保持扭矩，然后再发送读取指令；如果松开后才读取，数值会回到接近零。
...
有效数据为 `0001`，相对于去皮后的 `0000` 发生变化。这证明以下链路均已打通：

`传感器 -> 数字变送器 -> RS485 A/B -> USB 转换器 -> 树莓派串口 -> Modbus 指令`

注意：`0001` 只能认定为原始显示值 1。没有确认传感器量程、小数点位置、符号方向和标定关系前，不能直接写成 `1 N·m`。
